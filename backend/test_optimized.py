    ...
    with open("../HackitAll2025-main/eval-platform/src/main/resources/liquibase/data/flights.csv", 'r') as f:
        for row in csv.DictReader(f, delimiter=';'):
            ...
            flights_by_hour[(day, hour)].append({
                ...
                'origin': airport_id_map.get(row['origin_airport_id'], row['origin_airport_id']),
                'destination': airport_id_map.get(row['destination_airport_id'], row['destination_airport_id']),
                'aircraft_type': aircraft_id_map.get(row['sched_aircraft_type_id'], row['sched_aircraft_type_id']),
...
def main():
    ...
    print("Running OptimizedStrategy...")
    strategy = OptimizedStrategy()
    costs, counts = run_simulation(strategy, airports, aircraft, flights)
    
    print("\n" + "=" * 50)
    print(f"Loading:     ${costs['loading']:>15,.0f}")
    print(f"Movement:    ${costs['movement']:>15,.0f}")
    print(f"Processing:  ${costs['processing']:>15,.0f}")
    print(f"Purchase:    ${costs['purchase']:>15,.0f}")
    print(f"Unfulfilled: ${costs['unfulfilled']:>15,.0f}")
    print("-" * 50)
    print(f"TOTAL:       ${costs['total']:>15,.0f}")
    print("=" * 50)
    print(f"Loaded: {counts['loaded']:,} | Purchased: {counts['purchased']:,} | Unfulfilled: {counts['unfulfilled']:,}")
